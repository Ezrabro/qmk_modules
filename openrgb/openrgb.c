...
#endif
#if RAW_EPSIZE != 64
#   error "RAW_EPSIZE is not 64"
#endif

ASSERT_COMMUNITY_MODULES_MIN_API_VERSION(1, 1, 1);

// Direct mode color buffer
RGB g_openrgb_direct_mode_colors[RGB_MATRIX_LED_COUNT] = {0};
...
// HID data buffer for holding packets
...
bool raw_hid_receive_keychron(uint8_t *data, uint8_t length) {
    // Not an OpenRGB command — let Keychron's own handler take it
    if (data[0] < OPENRGB_GET_PROTOCOL_VERSION || data[0] > OPENRGB_DIRECT_MODE_SET_LEDS) {
        return true;
    }
    ...
    return false; // handled here, don't let Keychron also process it
}
...
void openrgb_get_device_info(void) {
    ...
    // Copy in the product string in up to the first half of the message
    uint8_t current_byte = 3;
    for (uint8_t i = 0; (current_byte < ((RAW_EPSIZE - 2) / 2)) && (PRODUCT[i] != 0); i++) {
    ...
    // Copy in the manufacturer string
...
void openrgb_get_mode_info(void) {
    ...
    if(mode == RGB_MATRIX_COMMUNITY_MODULE_OPENRGB_DIRECT) {
        ...
        // Correct for some reordered modes (Rainbow Moving Chevron, Cycle Out In, Cycle Out In Dual)
        if (mode == 15) mode = 17;
        else if (mode == 16) mode = 15;
        else if (mode == 17) mode = 16;

        mode = mode;
    }

    raw_hid_buffer[0] = OPENRGB_GET_MODE_INFO;
    raw_hid_buffer[1] = mode;
...
void openrgb_get_enabled_modes(void) {
    ...
    for (int mode = 0; mode < size; mode++) {
        ...
        else if (mode == 17) mode = 15;

        raw_hid_buffer[mode + 1] = openrgb_rgb_matrix_effects_indexes[mode];
    }
}
...
void openrgb_set_mode(uint8_t *data) {
    ...
    if (mode > 0) {
        ...
        if (save == 1) {
            rgb_matrix_mode(RGB_MATRIX_COMMUNITY_MODULE_OPENRGB_DIRECT);
        }
        else {
            rgb_matrix_mode_noeeprom(RGB_MATRIX_COMMUNITY_MODULE_OPENRGB_DIRECT);
        }
    }
